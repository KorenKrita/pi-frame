 ...
 * statusline settings live in ~/.pi/agent/pi-frame/statusline.json. Until that
 * file exists, pi-topping-statusline's settings.json is read as the start point.
 */
...
const settingsPath = () => path.join(getAgentDir(), "pi-frame", "statusline.json");
const legacySettingsPath = () => path.join(getAgentDir(), "pi-topping-statusline", "settings.json");
...
export function saveSettings(settings: StatusLineSettings): void {
	...
	fs.writeFileSync(`${file}.tmp`, `${JSON.stringify(settings, null, "\t")}\n`, { mode: 0o600 });
